...
- 候选不是最终裁决；证据够但分类或适用范围有疑义时，用 `assertion`＝“推”或“存疑”，不要直接删掉。
...
- D 世界规则：可重复适用的“如果—就会”；一次性场景和个人感叹不收。若原文已有条件＋通常结果，但适用范围还不确定，密候选阶段可用“存疑”入池。
...
- 正例：正文若明确写出“某组织从讨论方案变为正式签署并开始执行”，这是可被后文读取的 A 状态变化；再从本章证据目录选真实 ID。
- 反例：角色一时害怕、惊讶或感叹，不等于可重复的 D 规则；但原文明确给出“在某类条件下通常发生某结果”时，不能只因范围未定就静默删除，可标“存疑”。
...
按 A→B→C→D 完整扫描。先把有锚候选写进 `records`，再为真正为空的类型写 `coverage_audit`。

输出前按原始事实去重：每个 A 只保留一个状态变化；若同一预告已作为 B 记录其待执行动作，不得仅因读者会等待该动作再立 C，除非正文另有直接锚明确吊起一个不同于该动作本身、且跨章不回答会明显失望的独立问题。

只输出 JSON；不猜测；不复制示意；所有 `anchor_id` 必须来自本章目录。
