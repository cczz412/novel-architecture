...
映射只能使用 exact Patch operation、Diagnostic／Coverage／opaque SourceSlice ref 和 lineage／evidence locator。映射不完整或不唯一时扩大检查，不能读正文或用模型猜。
...
- B-05 不读取 B-03 SourceSlice bytes，也不重新判断正文含义。
