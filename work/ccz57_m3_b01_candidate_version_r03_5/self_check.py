...
class _WriterVisitor(ast.NodeVisitor):
    def __init__(self) -> None:
        ...
        self.evidence_locator_call_classes: list[str | None] = []
    ...
    def visit_Call(self, node: ast.Call) -> None:
        if isinstance(node.func, ast.Name):
            ...
            elif node.func.id == "_make_evidence_locator":
                self.evidence_locator_call_classes.append(self.class_name)
...
def writer_audit() -> dict[str, Any]:
    ...
    if visitor.locator_call_classes != ["CandidateVersionStore"]:
        conflicts += 1
    if visitor.evidence_locator_call_classes != ["CandidateVersionStore"]:
        conflicts += 1
    ...
    return {
        ...
        "evidence_locator_constructor_classes": (visitor.evidence_locator_call_classes),
...
def verify_object_shapes() -> dict[str, Any]:
    ...
    if catalog["catalog_version"] != CONTRACT_VERSION:
        raise AssertionError("OBJECT_SHAPES contract version drift")
    if catalog["candidate_schema_id"] != CANDIDATE_SCHEMA_ID:
    ...
    validate_candidate_version(candidate, reference_records=all_records)
    pointer = next(
        record
        for record in records
        if record["record_type"] == "M3_CANDIDATE_POINTER_SNAPSHOT"
    )
    validate_pointer_snapshot(pointer, records=all_records)
    validate_record_ref(catalog["record_ref_example"], records=all_records)
    validate_lineage_locator(
        catalog["lineage_locator_example"],
        candidate_version=candidate,
        reference_records=all_records,
    )
    validate_evidence_locator(
        catalog["evidence_locator_example"],
        candidate_version=candidate,
        reference_records=all_records,
    )
    legacy_summary = read_legacy_candidate_version(catalog["legacy_candidate_example"])
    if legacy_summary != catalog["legacy_read_summary_example"]:
        raise AssertionError("OBJECT_SHAPES legacy read summary drift")
    ...
    expected_map = {
        ...
        "M3_EVIDENCE_LOCATOR": "CandidateVersionStore constructor only; no persistence",
...
def run_self_check(*, allow_missing_manifest: bool) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ccz57-b01-self-check-") as temporary:
        ...
        restart_report_evidence = {
            "harness_process_calls": restart_process_evidence["harness_process_calls"],
        ...
        for phase in ("prepare", "readback"):
            phase_evidence = restart_process_evidence[phase]["runtime_event_evidence"]
    ...
    report = {
        ...
        "mechanical_pass": (
            len(normal_results) == len(NORMAL_SCENARIOS)
            and len(failure_results) == len(FAILURE_SCENARIOS)
            and static_pass
            and runtime_pass
            and vectors_pass
            and integrity_pass
