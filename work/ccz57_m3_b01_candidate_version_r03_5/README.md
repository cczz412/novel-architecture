...
后续 Issue #231 增加了构造时冻结的 authority profile。离线夹具默认仍是 `FIXTURE_ONLY`；产品调用方只能由受控组装器选择 `PRODUCT_CANDIDATE_AUTHORITY`，请求正文不能临时换身份。

R02 又收紧了组装入口：普通 `FixtureStore` 即使被传入产品 profile，也会在创建 `state.json` 之前拒绝。产品 B01 只能先进入不持久化的受控 capture，再由 `CandidateAuthorityStore` 一次性落库。
