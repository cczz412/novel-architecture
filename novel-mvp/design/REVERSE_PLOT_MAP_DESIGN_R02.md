# 反向剧情图管线设计稿 R02（REVERSE_PLOT_MAP_DESIGN）

身份：**轻档设计稿。** 第二版按 R13 对齐。接力点是现算投影，可以改；进真值须作者确认。历史稿 [REVERSE_PLOT_MAP_DESIGN_R01.md](REVERSE_PLOT_MAP_DESIGN_R01.md) 保留不动。回答一件事：**作者导入的存量章节，怎么变成规划账里可用的「影子章槽」**。不是施工合同：字段沿用 [PLAN_LEDGER_STORAGE_DESIGN_R03.md](PLAN_LEDGER_STORAGE_DESIGN_R03.md)。
...
| **追加导入章的线归属** | 后续再导旧章时，章影调用的输入带现有线清单，顺带给「建议归哪条既有线」候选 | 首轮之后不重跑线初建；日常新写的章不走本管线（走移交，线归属由创作流出题时定） |
...
| 接力点第一屏 | ❌ 投影 | M5/M9 界面层 | 现算聚合（见下表） | 作者 | **现算、可改；进真值须确认**，不另开第二真源 |
...
2. **接力点可改，进真值须确认**：纯投影，点任何一行可以改观感／纠错；改完进账必须作者确认。画布仍是真身（ADD-006），投影上不要另开第二本真值。
...
- B. 不给：接力点先出简版（章数／字数），完整反推另开；收费数字仍冻，不拿付费墙当开关
