# 「故事真源引擎」灵感账设计稿 R01（IDEA_LEDGER_DESIGN）
...
```text
raw 新记 ──安放──▶ placed 已安放 ──产物全部兑现──▶ realized 已落地
   │                                                （指向兑现章节）
   ├──搁置──▶ shelved（可留唤回条件）──唤回复核──▶ raw
   └──作废──▶ voided（留痕不删）        placed/shelved 同样可作废
```

- **placed 之后，进度不归灵感卡管**——归转化产物管（H 的 `hook_status`、MC/PE 的消化状态，全是现成机制）。灵感卡只显示一个派生角标：机械查产物状态汇总（「铺垫 2/2 已写，揭示待写」）。
- **realized 的翻转是机械可判的**：产物全部 `digested`/`paid` 时自动翻，留痕＋可手动改回。灵感账不是真值账，自动翻无害；翻的那刻回填 `realized_in`（兑现落在哪章）——「定期能让这个人物出场」的最终交代。
...
依据「素材货架＋借阅记录」（捞货 51）：已安放/已落地的灵感再次被点开（或作者想再用同一个点子）时，**卡面首行就是去向**：「已用过：安在 S-0012，伏笔 H-0021，第 12 章已兑现」。对 placed/realized 的灵感再点「找位置」，先弹这一行再问「真要再安一次？」——防同一个念头被忘了用过、重复安放成两条撞车的戏。
...
一段话说清（防第二真源、防混账）：**灵感账是愿望清单，规划账是计划清单，事实账是已发生清单——三本账三种身份，编号前缀分家（IDEA-/PE-·H-·MC-/F-），永不混写。** 灵感想进故事只有一扇门：安放动作（作者显式确认）生成规划账对象、`origin_ref` 回指出处；从那之后进度全归规划账的现成机制管（消化清单、伏笔状态、容量灯），灵感卡只做被动汇总显示，不产生第二套进度真源。挂件库跟灵感账的关系是借书不搬家：完善时引用词条 key，词条本体永远住在挂件词表里。事实账与灵感账互不引用——灵感永远不是「已发生」的证据，事实也永远不需要向愿望对账。
...
| `attach_refs` | list[obj] | 是（可空表） | 挂靠对象，每条 `{kind, value}`；kind＝`character`（人物名字符串，人物系统未建）／`storyline`／`slot`／`hook`／`free` | `[{"kind":"character","value":"陈九棺"},{"kind":"storyline","value":"L-0001"}]` |
| `status` | enum | 是 | `raw` 新记／`placed` 已安放／`realized` 已落地／`shelved` 搁置／`voided` 作废（留痕） | `"placed"` |
...
| `placement` | obj/null | 是(可 null) | 安放记录 `{anchor_slot_ref 揭示/主安放位, position_hint, timing_grade 五档, reason, produced 产物清单[{ref, slot_ref, note}], depth 轻/重, decided_by, placed_at}` | 见 JSON |
| `realized` | obj/null | 是(可 null) | 落地回填 `{realized_at, realized_in 兑现章位}`——产物全部兑现时机械翻 | 见 JSON |
...
3. **寻位任务用自己的小包，不借 C9 全套。** 作者点「找位置」时，编一个寻位小包：灵感全文＋完善稿最新版＋候选范围的胶囊层材料（各候选槽的章胶囊、线状态与上次现场、容量灯读数、相关伏笔的埋点/回收位——D1/D2 档为主）。这就是「寻位时才被查」的正面形态：查也只查胶囊层，不拖全账。
