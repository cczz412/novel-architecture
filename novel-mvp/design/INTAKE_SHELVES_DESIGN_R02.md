# M1 导入器·材料架分拣设计稿 R02（含 C1 v1 合同建议）

身份：**轻档设计稿。** 第二版按 R13 对齐。黄金三章是独立工作台，不是免费额度；收费数字仍冻，本稿不拿付费墙决定入架。历史稿 [INTAKE_SHELVES_DESIGN_R01.md](INTAKE_SHELVES_DESIGN_R01.md) 保留不动。C1 v1 仍是**合同建议稿**，正式升版要等施工时改 `contracts/C1_CHAPTER_DOC.md`；标 ⚠️ 未调查 的解析细节，动手前必须先实测。
...
**O2｜水印处理（CZ 2026-08-20 已选保守版）**
正式口径是「只标候选、不阻断导入、不自动删改」。原上传和章节原文始终保留；作者明确确认后也只能生成派生清洁版，不能回写原文。当前候选判等只做「去行首尾空白后逐字相同」，不做全半角、大小写、URL 或近义句归一。候选永远不写成“已机械确认”。

这个选择的代价是：在作者处置前，「求收藏」类文字仍会随原文进入后续候选；但不会因为规则误判而永久丢掉正文。
