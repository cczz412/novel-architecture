...
| # | 改了什么 |
|---|---|
| 1 | 收工不再写入 `slot_status=closed`；该字段只表示交棒进度 |
| 2 | 表和收口流不再写「存入→移交」；入库只对照 |
| 3 | 收工不拿移交／审查／对账当完成条件 |
| 4 | 小抄是可改投影，确认后才写回规划账 |
...
| M8 | [M8_PLANNING_DESIGN_R04.md](M8_PLANNING_DESIGN_R04.md)（收口流＋精简条＋C7） |
...
| STORAGE | [PLAN_LEDGER_STORAGE.md](../contracts/PLAN_LEDGER_STORAGE.md)／[PLAN_LEDGER_STORAGE_DESIGN_R04.md](PLAN_LEDGER_STORAGE_DESIGN_R04.md) |
...
| 查 | 不查 |
|---|---|
| **覆盖**：本章每条 PE（计划事件），正文里有没有对应内容——写了没写 | 文笔、风格、节奏、描写方式 |
...
| **真值接力棒** | 存入→只对照；选「以这篇为准」才移交。不存→**不移交，大纲继续为真** | 同左 | **不移交，大纲继续为真** |
| 对账（P13） | 存入→可选（建议跑，不以交棒为前提；此时是兜底，未写桶预期为空）；不存→不跑 | 存入→可选（跳检时对账是唯一核对，仍不以交棒为前提）；不存→不跑 | 不跑（没料） |
...
| 4 | §6.2 裁决一 | 语义扩展：写作区存入＝入库默认只对照，选「以这篇为准」才交棒；无正文/不存收工不交棒。收工不写 `slot_status=closed`（本稿 4.1 裁决一） |
...
- 任务书八问逐项回对：①形态（§1：左小抄右净桌＋手动锚定裁决＋专注模式＋K5 落位）；②当场检测（§2）；③暗稿（§3）；④三路径收工（§4.1：没有正文也可以点收工；收工不写 `slot_status=closed`）；⑤存不存裁决（§5）；⑥M8 对接（§6）；⑦角色 ID（§7）；⑧开放问题 3 条（§8）——无缺项。
...
- 没发明新拍板：新字段全部标注提交存储稿升版（§6.2 第 8 条）；R04 禁止把 `slot_status` 扩成 `closed`；四键升级、C9 暗稿装载仍是建议。停点未新占号；词条交 N9。
