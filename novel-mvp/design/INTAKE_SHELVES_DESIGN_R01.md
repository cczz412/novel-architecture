# M1 导入器·材料架分拣设计稿 R01（含 C1 v1 合同建议）

身份：**轻档设计稿，施工第三单（M1 材料架分拣）的图纸。** 依据 CZ 2026-08-13 拍板（挂账本 ADD-002／ADD-003／ADD-005）、MVP 问题台账的真书教训（I-001～I-013）、两份 Notion 考古捞货。它不是施工完成报告：C1 v1 部分是**合同建议稿**，正式升版要等施工时改 `contracts/C1_CHAPTER_DOC.md` 并同步收发两端，本稿不动合同文件；标 ⚠️ 未调查 的解析细节，动手前必须先实测，别拿设计稿当胸脯拍。
...
第 3 节整节详述。入架动作：章节架＋大纲区 → `chapters.json`（v0 兼容投影，见 5.1）；书名卡／简介卡／类型标签／设定集 → `shelves.json`；材料包本体全程留在 `intake/`，当进货台账和事后改判的入口。

### 1.7 量闸（ADD-002 拍板的落地位）

- **免费项目**：识别出的正文章超过 3 → 按章号取最前 3 章入架，其余标 `parked`（搁置）留在材料包里。**搁置不是丢弃**：付费解锁后直接入架，不用重导。为什么免费给前三章：ADD-002 拍的免费层就是「导前三章看效果」，且黄金三章深读本来就是价值最高的一截（19 号捞货概念 24）。
...
| status | `candidate` 分拣中 / `shelved` 已入架 / `parked` 搁置（免费超量）/ `discarded` 已丢弃 |
...
1. **chapters.json 结构不变**：章节架单元确认入架后，照样投影成 v0 章节文档（id／title／kind／text／added_at 一个不少），只**加**字段不改字段——M2 只吃 `text`，M0 status 只读 kind／title／text，全都无感知。大纲区沿用 v0 已有的 `kind=outline` 投影，行为不变；
...
data/万鬼伏藏/
  ...
  chapters.json             # v0 兼容投影：章节架＋大纲区（结构不变只加字段）
