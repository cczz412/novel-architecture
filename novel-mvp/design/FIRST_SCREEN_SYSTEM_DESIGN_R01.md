# 首屏体系设计稿 R01（FIRST_SCREEN_SYSTEM_DESIGN）

身份：**轻档设计稿，给 CZ 过目。** 管的是「第一眼」：三层各自的第一屏＋层间怎么进怎么退＋接力点第一屏正式稿（GAP-12 正面填）＋关章庆祝的形态。不动已有稿的机制——[M8_PLANNING_DESIGN_R01.md](M8_PLANNING_DESIGN_R01.md) 管选线之后的一切，[REVIEW_OVERVIEW_DESIGN_R01.md](REVIEW_OVERVIEW_DESIGN_R01.md) 管审查态，[REVERSE_PLOT_MAP_DESIGN_R01.md](REVERSE_PLOT_MAP_DESIGN_R01.md) 管接力点的货源；本稿只管这些东西**摆在谁眼前、按什么顺序出现**。

全文界面元素、按钮文案、系统提示语一律是**示意文案，非定稿**（这句全文只说这一次）。

拍板依据：挂账本（小说架构仓，路径含中文，复制用）`/Users/a1234/挣钱/小说架构/TEMP/bgboard-audit-20260809-r01/18_R07_ADDENDUM_LEDGER_20260813_R01.md`——核心吃 **ADD-012 F5**（三层递进基调）、**F2**（概览=感官愉悦）、**F6**（关章庆祝）、**F7**（积分淡显）、ADD-006（画布真身／点哪问哪／聊天框落画布）、**ADD-016 H1**（开章先选线）＋H3（章级沙箱）；捞货 38（接力点六要素）路径同上目录 `19_NOTION_V2_SALVAGE_20260813_R01.md`。
...
- **层 0 书架主页**＝「被等待着」：昨天你写到陈九棺进了义庄，接下来……
- **层 1 书画布（大纲编辑台）**＝「回到故事世界」：进书第一眼是自己的故事，不是待办清单
- **层 2 章工作台**＝「今天的活清清楚楚」：接力点六要素→选线→摆桌开工

```mermaid
graph LR
    HOME["层0 书架主页<br/>被等待着"] -->|点书卡进书| BOOK["层1 书画布·大纲编辑台<br/>回到故事世界"]
    HOME -.->|"点［继续写第N章］跨层直达"| HALL
    BOOK -->|"点［继续写第N章］"| HALL["层2 门厅<br/>接力点六要素＋选线(H1)"]
    HALL -->|"选定线"| DESK["层2 开工态（章级沙箱）<br/>摆桌→编排→出题→铺满<br/>（M8 稿地盘）"]
    DESK -->|"收工卡(F6)"| BOOK
```

核心设计三句话（CZ 快读）：

1. **接力点是一份现算投影，三层各取一个浓度**：主页书卡只取「写到哪＋上次收在＋今日可做一行」，编辑台把「写到哪」画成故事横带，工作台门厅才给六要素全量——同一份账，三个镜头，不重复造三套。
2. **工作台在 M8 的摆桌屏前面加一间「门厅」**：先看接力点六要素（你走到了哪），再选故事线（H1：选完才准备内容），选定那一刻才装包摆桌——M8 稿的「要点汇聚第一屏」从此是**选线之后**的第一屏。
3. **庆祝完不赶人**：收工卡亮出本章收工＋全书长一格＋连续天数，所有「继续写」入口在那一刻悄悄翻向下一章，但人留在原地自己决定走不走——翻页的是账，不是人。

## 1. 总则：一份接力点，三个镜头

先立一条防重复施工的纪律。接力点（写到哪／上次收在／主线／出场者／未结事项／风险／下一步方向，捞货 38 六要素）是**一份不落盘的现算投影**（反推稿 §6：纯投影、不可编辑、点任何一行跳回账本本尊）。三层不是三个功能，是同一份投影的三个浓度：
...
**【可点】** 点卡片本体＝进这本书的层 1（回到故事世界）；点主按钮＝跨层直达该书层 2 门厅（最高频动作走最短路，见第 6 节导航）；底部「开新书／导入旧稿」＝走导入分拣（INTAKE）。
...
**裁定：只有一本书时，打开产品直接落层 1（书画布），不再路过书架。** 被等待感不丢——「昨晚你写到……」那两行搬到层 1 顶部当欢迎条（见 3.1）。书架仍然存在（面包屑第一级「书架」，点得回去，开第二本书的入口在那）。

为什么：主页的本职是**多本书之间的选择**；只有一本书时选择不存在，硬留一屏就是每天多收一次过路费。日更是每天一次的高频动作（走查第 2 天定型的六件套第一件就是「看接力点，10 秒」），这一跳省得值。F5 的三层是三种情绪，不是三道必经闸门——单书用户的「被记得」由层 1 顶部承担，情绪一个不少。

顺带更正一处时序：走查屏 1 演的「打开产品第一眼＝接力点全量卡」成稿于 F5 三层拍板之前；三层落定后，接力点全量的家在层 2 门厅（第 4 节），打开产品的第一眼变成层 1 的故事世界。走查那屏的六要素形态和实测感受全部保留，只是搬了家。

### 2.3 空态（新用户，一本书都没有）

书架空态＝两条路的岔口（ADD-001 教程双路）：「看它怎么写一本书」（路 A 零材料演示教程，不花额度）＋「把你的章节交给它」（路 B 导入，粘贴/上传）。本稿不展开教程内页，只定：**空态不出现书卡骨架屏**——没有书就大大方方给两扇门，别摆一排灰框假装热闹。

## 3. 层 1 书画布（大纲编辑台）：「回到故事世界」

### 3.1 进书第一眼：故事横带在上，篮子在下
...
工作台分两态：**门厅**（本节，接力点第一屏正式稿＋选线）→ **开工态**（M8 稿地盘：摆桌→编排→出题→铺满，本稿不重复）。

### 4.1 门厅＝接力点第一屏正式稿（GAP-12 关闭）
...
| ⑤明显风险 | 一行，含通过态 | 有红黄报数＋点开进收件箱；没有就绿一句「✓ 无」（通过态要看得见，捞货 21）；不摊开明细 |
| ⑥下一步方向 | 底部动作区＝**选线卡** | 见 4.2——「下一步方向」不再是一个按钮，而是几条线各给一句方向 |

**【可点】** 点②③④任何一行＝跳回账本本尊（接力点不落盘不可编辑，改东西回画布改——反推稿 §6 防第二真源）；点⑤＝进收件箱；⑥区单选一条线；主按钮进开工态。

**【文案示意】** 未结事项的人话规则：伏笔写「底牌名——相位」，知情差写「谁还不知道什么」，改期欠账写「上章挪来的：××」。风险行 0 问题时固定「✓ 无（上次体检 0 红 0 黄）」。
...
4. **省心档少一步**：单线书或推荐线置信明显时，选线卡预选推荐项，作者一路点主按钮就过——选线是必经的闸，不是必答的题（对齐 ADD-005：小白默认放行，高级才处处停）。

### 4.3 开工态一句话（不重复 M8）

选线后进 M8 稿的完整流程：要点汇聚一屏（今天的活全摆桌上）→ 主架卡片 → 逐卡出题 → 铺满 → 复制精简条出门写正文 → 粘回收口。工作台头部常驻一行会话预估（F-4「本章规划约 40 积分」），左下角积分淡显（F7）——花钱的知情在门口一次说清，过程零弹窗。

## 5. 关章庆祝（F6）：收工卡＋翻页语义

### 5.1 形态裁定：一屏卡片，原地长出，不是弹窗

M8 §6.5 已定收工卡的内容（统计＋校准＋明日预览）和分寸（「拍拍肩膀，不是开派对」）。本稿补齐视觉层和翻页语义：

**【看见】** 三条件齐（移交✓审查清账✓对账无未处置✓，M8 §6.5 机械判定）时，收工卡在工作台原地长出——不是遮罩弹窗，是画布上的一张卡：

```
┌──────────────────────────────────────────────────┐
│ 🎉 第 5 章「守棺」收工！                               │
│                                                        │
│ ▓▓▓▓▓▒ ← 全书进度带：第 5 格点亮（0.8 秒柔光，一次）    │
│ 全书 5 章 · 16,100 字 ｜ 连续创作第 3 天 🔥             │
│                                                        │
│ 今天：正文 3,050 字 · 入账 31 条 · 计划兑现 3/3         │
│ 本章新埋：「仵作的警告」 ｜ 「族谱变淡」进入预热期       │
│ 顺手校准：预估 3,000 / 实写 3,050——挺准，不用调        │
│ ─────────────────────────────                          │
│ 明天的桌子：第 6 章——2 件必办 · 4 个候选（1 条正当时）  │
│                                                        │
│     ［ 看一眼第 6 章 → ］      ［ 今天到此为止 ］        │
└──────────────────────────────────────────────────┘
```

**动画标注（全部一次性，不循环）**：①进度带新格子点亮，0.8 秒柔光；②连续天数的 🔥 轻跳一下；③同一时刻，层 1 故事横带末端长出第 5 章画面卡（作者下次路过编辑台会看见故事长了一格——这是庆祝的**第二现场**，零额外设计，横带本来就会长）。不做烟花、不做全屏彩带、不出声——日更是每天都过的日子，庆祝要经得起天天看。

### 5.2 「庆祝完自动翻到下一章接力点」的语义：翻账不翻人

收工卡亮起那一刻，**所有入口悄悄翻页**：主页书卡变「继续写第 6 章」、层 1 主按钮同步、门厅六要素已按第 6 章现算（接力点本来就是现算投影，翻页零成本）。但**人不被搬走**：

- 点「看一眼第 6 章 →」＝进第 6 章门厅（接力点第一屏），明天的活先看一眼再走——这就是任务书说的「庆祝完翻到下一章接力点」的主动路径；
- 点「今天到此为止」＝回层 1 编辑台，落点定位在横带末端那张新卡上——收工的最后一眼是自己的故事世界，不是下一张任务卡。

为什么不自动跳转：F6 的心理原型是日更打卡——打完卡的下一步该是「舒一口气」，不是「下一个任务已为您加载」。翻页翻的是账（入口都指向明天），走不走是人的事。
...
- **返回＝上一层**（工作台→编辑台→书架），不是浏览器式「从哪来回哪去」。跨层直达（主页按钮直进门厅）的人点返回，落的也是编辑台——故事世界永远在中间，直达的人退一步也知道中间有个家。
- 聊天框跨层跟随（ADD-006 万能入口），焦点标签随层切换：全书／某章／某卡。

### 6.2 跨层直达与快捷动线

| 动线 | 路径 | 为什么给 |
|---|---|---|
| 日更快道 | 主页书卡按钮 → 门厅 →（预选线）→ 摆桌 | 走查定型的六件套从「看接力点」开始，最高频动作两次点击到位 |
| 回味道 | 主页点卡本体 → 编辑台横带 | 想看看自己的书，不想干活——这条路必须存在且不绕 |
| 收工回程 | 收工卡「到此为止」 → 编辑台横带末端 | 5.2 的裁定 |
| 粘贴新章 | 门厅／编辑台常驻「粘贴新章」入口 | 走查屏 8 的既有形态，不动 |
...
- 挂账本（小说架构仓，路径含中文，复制用）：`/Users/a1234/挣钱/小说架构/TEMP/bgboard-audit-20260809-r01/18_R07_ADDENDUM_LEDGER_20260813_R01.md`——**ADD-012 F5**（三层基调，本稿宪法）／**F2**（概览=感官愉悦＋抖音解说卡两出口）／**F6**（关章庆祝）／F7（积分淡显）；ADD-016 **H1**（选线是开章第一动作）／H3（章级沙箱多开）；ADD-006（画布真身三原则）；ADD-005（省心档预选放行）；ADD-002 Q5（手机伴侣范围）；ADD-013 G6（手机三件）／G7（体检只属导入侧）；ADD-001（教程双路→空态）；ADD-014（插件分层→门厅不出理论长文）。
- 捞货（同目录，复制用）：`.../19_NOTION_V2_SALVAGE_20260813_R01.md`——概念 38（接力点六要素＋导入后 15 分钟）、21（零唠叨＋通过态要看得见）、23（灯和说明分开）、47（不暴露失败码）、56（手机分工）。
- [DAILY_LOOP_WALKTHROUGH_R01.md](DAILY_LOOP_WALKTHROUGH_R01.md)：屏 1（接力点卡的实测感受与形态底稿）、屏 11（接力点翻页）、GAP-12（本稿正面关闭）、六件套动线。
- [M8_PLANNING_DESIGN_R01.md](M8_PLANNING_DESIGN_R01.md)：§1.1–1.3（摆桌/货架/触发相位——门厅④的相位和「明天的桌子」货源）、§6.5（收工卡内容与判定，本稿只补视觉与翻页语义）、§7（F-4 会话预估）。接线增补一条：汇聚输入加 `storyline_choice`（4.2），提交其 R02。
- [REVERSE_PLOT_MAP_DESIGN_R01.md](REVERSE_PLOT_MAP_DESIGN_R01.md)：§6（六要素货源表＋接力点不落盘纪律）、§2.2（exit_hook／在场人物货源）、§3.1（未放行降级态）。
- [REVIEW_OVERVIEW_DESIGN_R01.md](REVIEW_OVERVIEW_DESIGN_R01.md)：§1.1（审查态第一屏）、§1.3（三层递进）、§1.6（通过态）、§1.9（手机审批流）、§3.3（收件箱是 AI 唯一开口处）、§6（配图降级阶梯——横带画面卡直接复用）。
- [OUTLINE_STRUCTURE_DESIGN_R02.md](OUTLINE_STRUCTURE_DESIGN_R02.md)：五层篮子＋三横架（层 1 画布本体）、卷折叠（O2 的 B 选项依据）。
- [STOP_POINT_REGISTRY_R01.md](STOP_POINT_REGISTRY_R01.md)：P8（概览重生成）、P16（灵感）、F 类原则；[IDEA_LEDGER_DESIGN_R01.md](IDEA_LEDGER_DESIGN_R01.md)：手机记灵感入口。
- [INTAKE_SHELVES_DESIGN_R01.md](INTAKE_SHELVES_DESIGN_R01.md)：空态导入入口＋粘贴新章。
...
- 与既有稿冲突检查：M8「进工作台第一屏＝摆桌」与 H1 选线前置的时序冲突已显式更正并标注提交 R02（4.2 第 2 条）；REVIEW 审查态第一屏与本稿日常态的关系已裁定为同画布两态（3.2），不新增组件；接力点纯投影/不落盘与反推稿 §6 一致；六要素货源未新增任何账本字段。
