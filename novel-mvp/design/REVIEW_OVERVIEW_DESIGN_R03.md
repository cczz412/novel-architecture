# M5 审查台＋M9 故事概览 设计稿 R03

身份：**轻档设计稿。** 第三版清旧句。投影上可以改；进真值必须作者确认。本版 C5 先只承载事实概览；规划概览等完整 `plan.json` 读取合同后再升。历史稿 [REVIEW_OVERVIEW_DESIGN_R02.md](REVIEW_OVERVIEW_DESIGN_R02.md)／[R01](REVIEW_OVERVIEW_DESIGN_R01.md) 保留不动。不动 [OUTLINE_STRUCTURE_DESIGN_R01](OUTLINE_STRUCTURE_DESIGN_R01.md)／[R02](OUTLINE_STRUCTURE_DESIGN_R02.md)。
...
graph LR
    ...
    M9 -->|C5 概览卡·可改投影| M5["M5 审查台<br/>读概览→点开细节→签字"]
    M7["M7 一致性体检"] -->|C6 报告| INBOX["风险收件箱<br/>（M5 的一部分）"]
    M5 -->|确认/驳回/改写·唯一写入口| M4
```

一句话读图：M9 把事实账投影成「章概览卡」（C5，可改投影，进真值须确认），M5 摆在画布上让作者读；体检报告（C6）进风险收件箱集中处理；作者的确认/驳回/改写是全线**唯一**能改事实状态的口子，写回 M4。
...
1. **投影不是真源，但投影可改**：概览卡是 M4 事实账的快照，可物化、会过期作废；卡上不装事实句全文，只装 ID 引用（fact_refs），点开时现查 C4。作者可以在卡上改观感／纠错；**进真值必须确认**，禁止改完不经确认就当第二真源。出处：R13 ADD-061、ARCHITECTURE 设计纪律 1、捞货 29。
...
| basis | str | **本版只收** `fact_snapshot`（从 C4 生成）。`plan_snapshot` 等完整规划账读取合同后再升，不得从 C7 v0 出题切片伪造整章计划篮 | 是 |
...
【一致性修订 20260813｜G7 口径；R02 按 R13 补一句】体检的家在导入口岸（ADD-013 G7）：本节的预检/体检全部发生在「导入→抽取→审查」这条进货动线上，属**进货检查**。❌ 不要写「产品内创作章的质量由生成时约束保证」——创作侧也要检查（关章六道门里有质检；新章对账可选）。**不存在定期/例行全书体检**。新章正文入库时的增量核对算可选一站（见 [M8 R04](M8_PLANNING_DESIGN_R04.md) §6.1 第⑥步）。§1.1 书封卡上的体检入口按同一语义理解。
...
- **规划/教程场景（basis=plan_snapshot）**：等完整规划账读取合同后再升。画面提示从规划账场对象取，不得用 C7 v0 出题切片伪造整章计划篮。
...
- [ARCHITECTURE.md](../ARCHITECTURE.md)：M5/M9 模块位、C5 合同位、设计纪律（投影可改须确认、程序验机械人验语义、计划不冒充事实）。
