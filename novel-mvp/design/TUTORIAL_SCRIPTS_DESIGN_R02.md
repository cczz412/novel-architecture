# 新手教程双路逐屏脚本 R02（TUTORIAL_SCRIPTS_DESIGN）

身份：**轻档设计稿。** 第二版按 R13 对齐。激活＝看见系统在工作（ADD-057），不是 90 秒确认一条事实。黄金三章是独立工作台，不是免费额度也不是教程三入口之一。收费数字仍冻，教程出口不要写成付费墙。历史稿 [TUTORIAL_SCRIPTS_DESIGN_R01.md](TUTORIAL_SCRIPTS_DESIGN_R01.md) 保留不动。

> 演示书仍按 ADD-017 J7：从新书书库前三章挑真书，不自拟《捡骨人》。选书与授权须另过。机制骨架（胶卷条、出题卡、伏笔锁、收工卡、放映厅、失败兜底）仍有效。
...
教程有两条证法，但**打开产品的主入口不是这三扇门**。已有小说项目的人，进的是项目第一屏（梗概＋概览＋战报）。黄金三章工作台是另一张台：只打磨前三章，满意后再移交进项目。

- **路 A**：预制放映，证明系统在干活——表、人物、箭头、故事走向跟着成形（ADD-057）。90 秒只是模型别太慢，不是「确认一条事实才算激活」。
- **路 B**：用作者自己的前三章，走黄金三章工作台看效果。确认事实可以是首价值，但不能当唯一激活定义。
- **点子路**：可以探索，不能定义小说项目主产品，也不要一句话出 12 章成文。

两条路的终点互相指。教程出口是「进黄金三章工作台／进已有项目」，**不要写「继续写第 4 章（付费）」**。收费冻结期间，不拿付费墙当教程完成态。

```mermaid
graph TD
    LAND["已有项目 → 项目第一屏"] 
    G3["黄金三章工作台"]
    LAND2["空态：黄金三章 / 导入 / 看演示"]
    LAND2 -->|"先看它怎么干活"| A["路 A·固定演示"]
    LAND2 -->|"打磨前三章"| G3
    A -->|"用你自己的试试"| G3
    G3 -->|"满意后移交进项目"| LAND
```
...
- **路 B 不是独立产品面**，它叠在**黄金三章工作台**上：导入前三章→抽取→看见系统在工作→作者确认。不要把它做成「免费层＝前三章额度」。满意后才移交进小说项目。
...
| 确认动作 | **演的**：按钮照点，但不写任何账 | **真的**：确认仍是真值签字；激活本身是「看见系统在工作」，不把确认一条事实当成唯一激活事件 |
...
> ⚠️ 总稿03 洞-03 原样立着：抽取耗时没有端到端实测读数（抽取调 API，密度高的段还会截断重试）。90 秒只是延迟预算，不是「确认一条事实才算激活」。本屏的「第一段抽完即推」是把等待从「等全部」改成「等第一条」——激活发生在他看见表、人物、箭头在动的时候。仍需实测。
...
**【感受】** 它说的每句话都能指回我写的字。确认仍有分量。但激活已经发生在更早：抽取时表、人物、箭头在动，他看见系统在干活——不是「确认这一条才算用起来」。
...
- 👉 推荐 A：有稿用户别让他多点一下；零材料用户视线扫到角落的「看演示」成本极低；C 会把有稿用户看见系统干活的时刻推迟 3 分钟。❓ 待拍（总稿03 首屏形态本就标候选）
...
- 同目录设计稿：[DAILY_LOOP_WALKTHROUGH_R03.md](DAILY_LOOP_WALKTHROUGH_R03.md)、[INTAKE_SHELVES_DESIGN_R02.md](INTAKE_SHELVES_DESIGN_R02.md)、[REVIEW_OVERVIEW_DESIGN_R02.md](REVIEW_OVERVIEW_DESIGN_R02.md)、[REVERSE_PLOT_MAP_DESIGN_R02.md](REVERSE_PLOT_MAP_DESIGN_R02.md)、[SCENE_CARD_EXPORT_DESIGN_R01.md](SCENE_CARD_EXPORT_DESIGN_R01.md)、[M8_PLANNING_DESIGN_R03.md](M8_PLANNING_DESIGN_R03.md)、[STOP_POINT_REGISTRY_R02.md](STOP_POINT_REGISTRY_R02.md)。
- 捞货38（复制用）：`/Users/a1234/挣钱/小说架构/TEMP/bgboard-audit-20260809-r01/19_NOTION_V2_SALVAGE_20260813_R01.md`——接力点六要素＋15 分钟红线。

## 7. 轻档自查记录

- 任务六问逐条回对：①路 A 逐屏（A1–A9＋题材推荐❓＋时长 3 分钟＋转化出口 A9）；②路 B 入口（B1 粘/传合流＋C1 点子）、前三章路线按「导入→分拣→抽取→看见系统在干活→概览画面卡→体检→分镜→进黄金三章工作台／进已有项目」全序落屏、点子路线 C1–C5 按「点子→大纲引导简档→剧情层❓→概览＋分镜」落屏；③边界（1.2 演示态/导览层表：路 A 假签不写账、路 B 真签不豁免、可跳可重看、tutorial_state 用户级不进项目账）；④分镜呈现（1.1 胶卷条＋各屏落帧点：A4/A6/B5/B6/C3，拼接时刻 A8/B8/C4）；⑤失败兜底（第 4 节逐屏表＋路 A 总兜底）；⑥开放问题 3 个含题材题。
