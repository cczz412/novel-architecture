# 章工作台设计 R01（CHAPTER_WORKBENCH_DESIGN）——选线前置·章级沙箱·分层回退

身份：**轻档设计稿。** 拍板依据是 CZ 第三轮校准的三节原话——ADD-016 **H1**（选线是开章第一动作）、**H3**（章级沙箱天然支持并行推演）、**H7**（整章重写＝分层回退），加上 ADD-013 G3（要点汇聚一屏——选线在它上游）和 ADD-012 F8（驳回记录——回退记录的同族数据）。

与 [M8_PLANNING_DESIGN_R01.md](M8_PLANNING_DESIGN_R01.md) 的分工一句话：**M8 管流程每一步干什么（汇聚→编排→出题→铺满→收口），本稿管这些步骤发生在什么现场**——先选线才开工、整个工作现场是个沙箱、现场分三层随时可退。M8 R02 正在修订中；本稿与 M8 R01 冲突处（主要是「选定即落账」的落账位置，见 §2.1）以挂账本 ADD-016 为准，冲突调和已列成 M8 R02 修订提交清单（§5.5）。不是施工合同；字段表是人读版，Schema 随施工单机械翻译。例子沿用设计稿一族的《万鬼伏藏》，贯穿第 4 章「反派线 vs 主线」并行推演。

---

## 0. 一句话＋三句话

章工作台，说白了就是作者规划一章的**工作现场规格**：进门先回答「这章跟谁走」（选线），进来之后随便铺随便改随便反悔（沙箱＋分层回退），满意了一键过闸才记进正式规划账（转正）。

1. **选线前置（H1）**：开新章第一屏＝选故事线，选完系统才备料——续线导线状态、切线换出场表、开新线建三件套。「关注的内容变了就是新线」，系统在三种信号出现时主动问。
...
## 1. 选线前置（H1）：开新章第一屏＝选故事线

### 1.1 第一屏长什么样（纯机械，零调用）

CZ 原话的骨架：「每开新章先选故事线——给几条线供选，选完才准备内容，选不同线要准备的东西完全不同。」所以选线屏排在要点汇聚**之前**（M8 §1.1 的桌子是选完线才摆的），而且它是**纯机械屏**——候选列表和状态一句话全部从账本现拼，不花一个积分；汇聚调用在选线之后才开始花钱。
...
|---|---|---|---|---|---|---|
| P20 | 选线（开新章先选跟谁走） | 每次开新章工作台第一屏；开新沙箱时同屏复用 | 提醒不停（推荐线自动带入＋横幅可换；单线书体感零打扰） | 必停（列全候选自己挑） | 不设无痕自动档——选线决定整章备料，最少留横幅；新线信号／沉寂提醒只提醒不强停 | 本稿 §1〔候选，原 P18 改号〕 |
