# M8 续写规划详细设计 R04（M8_PLANNING_DESIGN）

身份：**轻档设计稿。** 第三版清旧句。计划不许叫已发生的事实。入库不自动交棒。对账不以交棒为前提。收工 ≠ 关章。字段以 [PLAN_LEDGER_STORAGE.md](../contracts/PLAN_LEDGER_STORAGE.md) 为准。历史稿 [M8_PLANNING_DESIGN_R03.md](M8_PLANNING_DESIGN_R03.md)／[R02](M8_PLANNING_DESIGN_R02.md)／[R01](M8_PLANNING_DESIGN_R01.md) 保留不动。

上承 [OUTLINE_STRUCTURE_DESIGN_R02.md](OUTLINE_STRUCTURE_DESIGN_R02.md)、[DAILY_LOOP_WALKTHROUGH_R03.md](DAILY_LOOP_WALKTHROUGH_R03.md)、**[PLAN_LEDGER_STORAGE.md](../contracts/PLAN_LEDGER_STORAGE.md)（字段合同）**／[PLAN_LEDGER_STORAGE_DESIGN_R04.md](PLAN_LEDGER_STORAGE_DESIGN_R04.md)（机制）、[CONTEXT_PACKER_DESIGN_R01.md](CONTEXT_PACKER_DESIGN_R01.md)、[STOP_POINT_REGISTRY_R03.md](STOP_POINT_REGISTRY_R03.md)、[WRITING_DESK_DESIGN_R04.md](WRITING_DESK_DESIGN_R04.md)。

行为看本稿；字段看规划账合同。第 8 节 C7 仍是出题快照的人读版，不是 `plan.json`。例子沿用《万鬼伏藏》。
...
| # | 改了什么 |
|---|---|
| 1 | §0 读图第三句：入库只对照，不再写「入库即移交」 |
| 2 | §6.3：对账读本槽当前可对照的 PE／H／MC，不以交棒为前提 |
| 3 | §6.3：相反档不再写「正文赢（默认）」；作者选往哪边改 |
| 4 | 计划事件不许再叫「计划事实句」 |
| 5 | 字段改指向已落规划账合同 |
...
3. **收口是一条流**：粘回→入库只对照→作者可另选「以这篇为准」才交棒→**本章异常单一屏**（审查、对账、暗稿重签分项落账，不能一个按钮代签）→对账边可在未交棒时落→收工卡或「保存先走」；未兑现项带着 defer_count 流进下一章汇聚屏。保存、交棒、审查、对账、关章各是各的。
...
| `planned_events` | 计划事件怎么动：`add[]`／`revise[]`／`void[]` | 直接执行（规划账是计划层，P1 选定就是授权） |
...
省心档与掌控档的区别只剩**提示形态**：省心档建议卡收在横幅一行不弹窗、作者有空再点；掌控档当场弹卡。**没有任何档位自动搬剧情。** P3 已写入 [STOP_POINT_REGISTRY_R03.md](STOP_POINT_REGISTRY_R03.md)。
...
交付合同不变：**「章计划精简条」＝`plan.json` 本槽的字段投影**；C7 只是当前出题切片，不是精简条的数据源。提示词工厂第二重身份（J1）不变。

【R02 修订（A10 替拍，2026-08-13）】**展示粒度口径注明**：精简条**默认「节拍档」**——本章目标／场序表（每场胶囊一句＋字数预估＋POV）／信息释放点／红线全量／出口钩子／写法指导一句。K1 拍板原话「本章核心内容其实很少」，130 条计划事件 ÷ 3000 字＝每 23 字兑现一条，默认摆出来就是逐句翻译器，压死人。**可展开「逐句档」**：点开某场展开该场 `planned_events` 拍全文（R01「拍全文不进精简条」收窄为「不进默认档」）；「复制章计划（纯文本）」同样两档，默认复制节拍档。这是展示层选择不动账本；CZ 若想默认句级，翻一个默认值即可（A10 替拍，留改判口）。
...
**裁决二（对账准入接缝补清）**：语义观察可以先拿 current C1＋冻结 C3 候选比计划，形成短命六态候选；正式 RE 的 confirmed fact 引用仍只能在 M5 作者确认之后接入。这样不让模型写 facts，也不要求作者先确认一堆与计划无关的候选。有稿才建议跑，作者可跳过；无正文收工不跑对账。

### 6.3 对账怎么比：落对账边（六态），三桶变归组视图【R02 重写（ADD-030.2，2026-08-13 替拍）】

**谁跑**：M8 收口件（对账回答「计划和书稿哪里不一样」，规划账的问题）。输入＝current C1 原文＋冻结 C3 候选＋规划账查该槽**当前可对照**的 PE／伏笔回收安排／MC。不以交棒为前置；当前实现只接已交棒 C1。

**怎么比**：一次小额模型调用做语义匹配，先产出 [RECONCILIATION_CANDIDATE v1](../contracts/RECONCILIATION_CANDIDATE.md)。程序验证 C1 逐字证据、计划 coverage 与 revision 后，才为每条可对照计划对象创建稳定 RE。新边起初不带 confirmed fact 引用，因此不支持 actual；作者走 [facts 准入动作](../contracts/RECONCILIATION_FACT_ADMISSION_ACTION.md) 后才接 C4。观察结果仍是背景板拍板六态：
...
**处置＝边上的 `author_decision`**（观察/处置两轴分开）。模型不能写它；facts 准入只处理 exact／variant／contradicted／unplanned 的书稿事实，unrealized 的 defer／void 与改稿继续走既有规划／写作入口：
...
- ⛔ 相反：不许静默；进异常单冲突栏，作者选「以这篇为准」（计划侧留痕）或改正文迁就大纲（任何档位显式——铁边界不变）。没有「正文赢默认」。
- ➕ 新增：提示登人物名单/故事线成员；处置后边留档。

**机械覆盖校验兜底**：该槽每条可对照 PE 必须有边，模型漏判的强制建 `ambiguous` 边——对账不许藏账。**改史自动撤回**从此接上机器层：某条 F 被改判→反向索引（存储 R02 §5.4）找到引它的边→边转 `stale`→吃这些边的派生结论（伏笔 fulfillment、人物 current 投影）自动回撤或标过期，等重对。对账调用计入会话包。
...
```
━━ 第 5 章异常单 ━━━━━━━━━━━━━━━━━━━━━━━
⛔ 冲突 1 ｜ 计划「爷爷遗忘九棺」——正文写的是师妹遗忘 → [以这篇为准·计划留痕] [改正文(显式)]
...
C7 v0＝一次出题快照，只覆盖写 `plan_latest.json`，不是章篮本体，也不是 `plan.json`。章篮住规划账。`status` 恒 `"plan"`。

### 8.2 字段表 v1

与 R01 一致，一处措辞修订【R02 修订（ADD-030，2026-08-13 替拍）】：`digested_refs` 的说明从「本章解决了哪些要点」订正为「本章**安排解决**了哪些要点」。给 M9 的展示口径：有对账边就从对账边取「实际兑现」（带 support_set）；没有对账边只显示「本章安排了…」。不要用交棒当开关，也不许拿 `digested_refs` 冒充兑现清单。`basis_note` 按存储 R02 §5.3 升级：双时点＋`story_commit_seq`。
