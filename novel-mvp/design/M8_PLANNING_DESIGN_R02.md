# M8 续写规划详细设计 R02（M8_PLANNING_DESIGN）

身份：**轻档设计稿。** 这是整个产品的心脏机制：要点汇聚→主架卡片→逐卡出题→选定即改账→事实句层铺满→作者写正文→回来收口。R01 全文保留不动（[M8_PLANNING_DESIGN_R01.md](M8_PLANNING_DESIGN_R01.md)），本稿是按第三册挂账本替拍决策（ADD-030／ADD-035）＋既有拍板 J5（ADD-017）返工、并已补 **ADD-036／ADD-037 亲拍**（目的挂卡、反向拆卡、写作指导＝副产品）后的现行版。

上承 [OUTLINE_STRUCTURE_DESIGN_R02.md](OUTLINE_STRUCTURE_DESIGN_R02.md)、[DAILY_LOOP_WALKTHROUGH_R01.md](DAILY_LOOP_WALKTHROUGH_R01.md)、**[PLAN_LEDGER_STORAGE_DESIGN_R02.md](PLAN_LEDGER_STORAGE_DESIGN_R02.md)（数据字段口径——对账边、handover parts、defer_count 的表设计在那边）**、[CONTEXT_PACKER_DESIGN_R01.md](CONTEXT_PACKER_DESIGN_R01.md)、[STOP_POINT_REGISTRY_R01.md](STOP_POINT_REGISTRY_R01.md)。
...
M8 干的事，说白了就是每天陪作者走这一圈：**先把「这一章要解决什么」全摆上一张桌子 → 作者像逛素材货架一样勾挑 → 系统把要点编排成几张主架卡片 → 逐卡出题给选项，每个选项折叠区里写好「选它账上怎么动」，选定那刻账就改完 → 铺成事实句层的章计划 → 作者去写 → 粘回来对账、翻页、道贺。** 作者的笔永远是自己的，M8 只管想清楚和记清楚。
...
graph TD
    ...
    DIS --> Q
    APPLY --> FILL["⑥ 铺满：事实句层<br/>场＋拍＋信息释放点"]
    FILL -->|"P3 容量灯·搬剧情须确认"| FILL
    FILL --> HANDOFF["⑦ 复制章计划精简条<br/>默认节拍级·可展开句级"]
    HANDOFF --> CLOSE["⑧ 收口流：粘回→移交→<br/>本章异常单一屏（ADD-035）→<br/>对账边落账（ADD-030）→收工/暂存"]
...
graph LR
    ...
    B --> C["③ 真值移交<br/>自动·handover parts<br/>（operation_id 包住）"]
    B --> D["④ 抽取<br/>自动后台"]
    D --> E["⑤ 审查放行<br/>作者签字·真值类"]
    E --> F["⑥ 偏差对账<br/>自动跑·落对账边"]
    E -.呈现合成.-> X["★ 本章异常单一屏<br/>低置信＋新增＋未兑现＋冲突<br/>一次签（重签类单签）"]
    F -.呈现合成.-> X
    X --> G["⑦ 增量体检横幅<br/>作者可选·P6+F-1"]
    X --> H["⑧ 收工卡 或 保存先走<br/>次日收口"]
    G -.可跳过.-> H
```

| 步 | 谁触发 | 干什么 | R02 变化 |
|---|---|---|---|
| ① 粘贴新章 | 作者点 | 粘贴框＋自动识别章号 | 不变 |
| ② 入库分拣 | 自动 | 单章走 P5 | 不变 |
| ③ 真值移交 | 自动，随入库连动 | 【R02 修订（ADD-030.4）】按存储 R02 §3.2 走：`handover_parts` 追加部件、只翻被覆盖场/拍、槽状态可 `partial`（一槽拆两章先写完一半也能如实记账）；整个动作一个 `operation_id` | 单值 handover 废除 |
| ④ 抽取 | 自动后台 | M2→M3 出候选 | 不变 |
| ⑤ 审查放行 | 作者（真值签字） | 章卡流审查 | 机制不变；**呈现并入异常单**（6.4） |
| ⑥ 偏差对账 | 自动跑（本章候选清账后） | 【R02 修订（ADD-030.2）】比对结果**落对账边实体**（六态），不再只是派生视图（6.3） | 呈现并入异常单 |
| ⑦ 增量体检横幅 | 作者可选 | G7 口径不变 | 不变 |
| ⑧ 收工 | 自动亮 | 【R02 修订（ADD-035.2）】两档：全清→收工卡；未清→「保存先走」暂存卡，次日收口（6.6） | 新增暂存档 |

### 6.2 两个悬了很久的口径（R01 已裁决，维持）

**裁决一**：粘贴入库与移交同一下，不加确认——移交是记账动作不是真值签字。R02 补强：有了 operation_id 护栏（存储 R02 §5.1），「入库连动移交」崩溃一半的余账也能恢复扫描清掉，裁决更稳了。

**裁决二**：对账拿「已确认事实」比计划，排在审查放行之后自动跑。不变。
...
**全清收工（机械可判）**：③移交完成 ∧ ⑤本章候选清账 ∧ 异常单无未处置项（省心档「自动正文赢＋标注挂着」视为已处置）→ 收工卡自动亮，接力点翻页。体检不是完成条件。

**保存先走**：③完成后任何时刻可点——正文与移交已安全落账，异常单挂为待办；收工卡换成暂存卡：
...
次日首页主按钮变「收口第 5 章异常单（4 项）」，直达 6.4 那一屏；收口完成才亮正式收工卡、翻接力点。连续创作天数按「写了正文」计，暂存不扣（打卡心理不被收口时间绑架）。
...
| 档 | 表现 |
|---|---|
| 省心（默认） | 对账自动跑完落边：兑现边静默；未写边自动 `accept_as_is`＋标注挂着＋角标可回访（defer 需作者点，不自动挪——挪期涨 defer_count，是有后果的决定）；新增/冲突进异常单等签 |
| 掌控 | 异常单弹出，逐项处置完才到收工卡 |
| 铁边界 | 反向动作（改正文迁就大纲）任何档位显式；对账**跑不跑**不分档——账必须对，档位只管处置停不停 |
