# 规划账存储层设计 R02（PLAN_LEDGER_STORAGE_DESIGN）

身份：**轻档设计稿。** 上承 [OUTLINE_STRUCTURE_DESIGN_R02.md](OUTLINE_STRUCTURE_DESIGN_R02.md)（概念定稿）；R01 全文保留不动（[PLAN_LEDGER_STORAGE_DESIGN_R01.md](PLAN_LEDGER_STORAGE_DESIGN_R01.md)），本稿是按第三册挂账本替拍决策（ADD-030／ADD-032）返工、并已补 **ADD-036 亲拍**（存储终点 PostgreSQL；SQLite 中转未拍）后的现行版。
...
M8 稿 §8.4 的配套账本增补清单（arc_card／point_board／触发字段／option_record 增补等）**本轮仍挂账**：等 [M8_PLANNING_DESIGN_R02.md](M8_PLANNING_DESIGN_R02.md) 定稿后随存储稿下一修订一次吸收，避免两头同时改同一批字段。

---

## 0. 三句话结论（CZ 快读）

1. **形状**：数据形状用一份与引擎无关的 JSON Schema 合同定死——本稿的十三类对象字段表就是它的人读版，上层模块照合同抓数，永远不关心底下是文件还是数据库。
...
| `slot_status` | enum | 是 | 【R02 修订（ADD-030，2026-08-13 替拍）】`planned` 计划中／**`partial` 部分移交**／`handed_over` 已移交／`dropped` 已作废（留痕不删）——`partial`＝一槽拆两章写、先写完的那半已入库 | `"planned"` |
| `handover_parts` | list[obj] | 是（可空表） | 【R02 修订（ADD-030，2026-08-13 替拍）】移交记录，**从单值改多部件**：每次「写成入库」追加一件 `{part_no, chapter_id, covered_scene_refs, covered_pe_refs, handed_at, decided_by}`；整槽状态由部件覆盖情况推导（见 §3.2）。原 `handover` 单值字段废除 | `[]` |
...
| `target_ref` | str/null | 条件 | 目标编号（`fact`／`setting` 时必填）——存对方**内部永久 ID**（§5.5） | `"F-0031"` |
...
| `actual_fact_refs` | list[str] | 是（可空表） | 成文侧证据：已确认事实 F 号；「未实现」态＝空表 | `["F-0044"]` |
...
三条配套规则不变：升级是真值签字（任何档位显式动作）；抽取指标只对影；偏差报告只对移交。
...
作者写完某章、正文入库（章节架新增 `c04`）那一刻，数据上这样走——**整个动作包在一个 `operation_id` 里（§5.1 护栏）**：
...
// RE-0001: {planned_ref: "PE-0412", actual_fact_refs: ["F-0044"], outcome: "variant",
...
{"id": "PE-0412", "truth_bearing": "handed_over", "digest_status": "digested",
 "deviation_note": "实际写成师妹遗忘（正文赢，作者已阅）", "defer_count": 0}
```

对账把偏差亮给作者、由他选往哪边改（默认正文赢）；机器可判的结果在对账边，`deviation_note` 只是人读备注。
...
**问题**：事实 F-0031 被改判，谁受牵连？R01 靠反扫全账——单跳还行，但「F→引脚→槽→该槽选择记录→衍生 PE→对账边」是多跳遍历，全表扫会指数爆且账里有环（拍↔伏笔互指）。
