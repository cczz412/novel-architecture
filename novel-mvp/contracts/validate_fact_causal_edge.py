"""Validate FACT_CAUSAL_EDGE fact-causal-edge-v1."""
...
SCHEMA_PATH = DIR / "FACT_CAUSAL_EDGE.schema.json"
FIXTURE_PATH = DIR / "FACT_CAUSAL_EDGE.fixtures.jsonl"
CONTRACT = "FACT_CAUSAL_EDGE"
VERSION = "fact-causal-edge-v1"


class ContractError(ValueError):
    """A FACT_CAUSAL_EDGE invariant failed."""
...
def validate_record(document: Any) -> dict[str, Any]:
    ...
    if document["confirm_status"] == "confirmed" and not document["evidence_refs"]:
        raise ContractError("CONFIRMED_REQUIRES_EVIDENCE")
    if document["from_fact_ref"] == document["to_fact_ref"]:
        raise ContractError("SELF_LOOP_FORBIDDEN")
    return document
