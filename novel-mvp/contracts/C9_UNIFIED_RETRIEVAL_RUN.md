...
带缺口继续只交付已经找到的材料，并明确列出缺口；即使这次一条材料也没找到，只要全部缺口都不是安全或完整性硬错误，仍按调用方交来的 `AUTO_CONTINUE`／`WARN_AND_CONTINUE` 返回空包和 `READY_WITH_GAPS`。核心不生成假设。下游若另有权限带假设继续，假设必须留在下游候选和未来返工接缝中，不能混进 C9 的取件材料。
