...
带缺口继续只交付已经找到的材料，并明确列出缺口。如果这次最终一条材料也没有装入——包括请求只有 SHOULD／MAY，而这些可选项全空、未读或全被预算遗漏——只要全部缺口都不是安全或完整性硬错误，就返回空包和 `READY_WITH_GAPS`，不能因为“没有 HARD”误报完整。可选材料实际装入时仍按正常规则判断 READY。核心不生成假设。下游若另有权限带假设继续，假设必须留在下游候选和未来返工接缝中，不能混进 C9 的取件材料。
