...
通用信封及转换入口未提供宿主targets清单时，只接受共同信封的九个字段路径，不能省略目标检查；六账内容宿主从自己的固定清单传入扩展路径。信封及旧夹具省略版本时固定按v1，显式null／数组／对象等非法版本返回合同错误并汇总，不能猜成v2。确认入口接受entry_kind（默认DEFINITION）；STATE／CHANGE按各自故事时间规则校验，夹具转交该声明。退役仍要求已有宿主的完整记录校验，不能把状态／变化条目冒充六账定义根或用局部对象替代正文快照；未落地宿主不在本轮补造。
