...
**正式版本：`ledger-read-tool-contract-v2`；兼容读取：`ledger-read-tool-contract-v1`**

一句话用途：让主 AI 和卡片工作流通过同一套受信只读入口，按精确版本读取十本账材料，并用可重放回执证明“这次读了什么”。

v1 请求和回执继续有效；v2 只增加受众和裁剪证明，不改变四态状态骨架、current／pinned 语义或失败原因。v2 回执必须记录 `audience`、`access_basis_sha256` 和 `projection_sha256`，并把三者纳入规范摘要。屏蔽发生在送出前，返回对象不能留下秘密字段的外壳；若屏掉世界规则的必要条件（`rule_text` 或 `exceptions`），就不能把剩余片段称为完整命题。

v2 三个字段的固定含义如下：交付对象（`audience`）只能是 `AUTHOR`、`MODEL`、`READER` 或 `PLUGIN`，由受信调用上下文绑定，不能靠请求自报获得权限；权限依据摘要（`access_basis_sha256`）绑定实际调用身份、任务许可和所用组规则；交付内容摘要（`projection_sha256`）绑定裁剪后真正送出的业务数据。精确计算规则见第 7 节。

本票只冻结 v2 文本；现有 Schema、校验器、fixtures 和读取运行代码仍为 v1。第二刀完成前不得宣称 v2 读取已开放，也不得把 v1 回执换个版本名当成 v2。
...
v1 计算方式原样保留。v2 的规范依据对象沿用上面的字段，把工具合同版本设为 v2，再增加交付对象、权限依据摘要和交付内容摘要三个字段；不能静默改变旧版摘要含义。

权限依据摘要的输入固定包含：受信操作身份（`trusted_actor`，不含登录令牌）、作者／项目（`author_id`、`project_id`）、本次任务许可（`task_grants`）、权限策略版本（`permission_policy_version`）、交付对象（`audience`）、读取档位（`read_profile`）、工具（`tool`）、规则版本（`rules_version`）、实际参与权限判断的组（`applied_groups`）。每个组以来源稳定 ID、修订号、组 ID 和完整组规则绑定；按来源 ID、修订号、组 ID 排序。集合型任务许可先排序去重。不能只哈希组名而漏掉组内容，也不能只绑定作者身份而漏掉实际调用者与任务许可。

交付内容摘要的输入就是送出的 `data`，包括业务正文；没有业务数据时哈希 JSON 的 `null`。不包含回执自身，避免摘要互相引用。两种摘要都采用本节的规范 JSON 序列化方法。权限依据只向受信端提供原件，对外回执给摘要，不透出被屏蔽的组和秘密字段名。

组裁剪必须在业务数据、公开来源清单和摘要输入定稿之前完成。被挡内容不保留空对象、占位符、秘密字段名或存在性说明。世界规则的规则句与例外（`rule_text`、`exceptions`）按同一必要字段组处理；其中一项不能交付时，两项一起屏蔽或拒绝把该对象作为完整规则交付。旧版引用仍需重新核验当前访问权，不能借读取历史恢复已撤销的权限。
