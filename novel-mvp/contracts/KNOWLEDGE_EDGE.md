...
v1 旧边对象、Schema、读取和 revision 语义继续兼容；本版只废止无现存边集合的 v1 新候选调用。v1→v2 迁移仍必须走以后单独批准的迁移流程，不能绕到新候选入口。
...
- start 与 `as_of` 都有 `story_order` 时独立检查下界，end 与 `as_of` 都有 `story_order` 时独立检查上界；数字结果优先，不能用相同修订引用覆盖与它矛盾的数字顺序。只对无法比较的一侧回退到完整 `chapter_revision_ref`；`as_of == start` 允许，`as_of == end` 拒绝，其余返回 `READ_GRANT_AS_OF_UNDETERMINED`。
