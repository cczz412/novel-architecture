...
r05 外置包有 40 个文件、702,031 字节；r06 有 41 个文件、714,081 字节。S-07-F-B-A
确认 60 个重件没有现役消费者后，从主仓移除 r05／r06 各 30 件，共 1,259,208 字节。
r05 当前留 10 件，r06 留 11 件；Z98 继续读取两轮请求体和用量账，r06 的正式成绩与请求
指针也继续保留。两轮消费者状态仍为 `open`，剩余轻量入口不能删除，也不能原地复跑。
旧 `receipt.md` 记录的是 2026-07-23 当时的完整现场，当前位置以各轮 `summary.md` 和
外置对象登记册为准。
