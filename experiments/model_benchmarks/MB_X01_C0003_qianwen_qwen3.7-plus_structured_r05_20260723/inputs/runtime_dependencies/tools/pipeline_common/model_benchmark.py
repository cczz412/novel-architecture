...
def load_adapters() -> dict[str, Any]:
    ...
    if adapters.get("schema_version") != "model-benchmark-provider-adapters-v1":
...
def resolve_profile(adapter: Mapping[str, Any], profile_id: str) -> dict[str, Any]:
...
def build_body(
    ...
    if body.get("n") != stage["invariants"]["n"]:
    ...
    diff = {
        ...
        "comparison_label": (
            f"{provider['provider']}／{model_id}／temperature=0.0／"
            "供应商兼容档条件成绩"
        ),
    }
    return body, diff
...
def prepare(
    ...
    *,
    ...
    profile = resolve_profile(adapter, profile_id)
...
def load_context(root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    profile = resolve_profile(adapter, str(spec["profile"]))
...
def audit_completed(root: Path) -> dict[str, Any]:
    ...
    model_data, content, reasoning, finish = response_envelope(response, str(spec["model"]))
...
def run(root: Path, *, opener: Any = None) -> dict[str, Any]:
    ...
    try:
        model_data, content, reasoning, finish = response_envelope(response, spec["model"])
        catalog_doc = read_json(root / "inputs/evidence_catalog.json")
        catalog = catalog_doc.get("entries") if isinstance(catalog_doc, Mapping) else None
        if not isinstance(catalog, list):
...
本轮是“{spec['profile']}＋temperature=0.0”条件成绩。供应商兼容差异见 `prepared/compatibility_diff.json`，不能把它冒充成所有参数完全相同的纯模型单变量。
...
def list_options() -> dict[str, Any]:
    ...
    for provider_id in sorted(adapters["providers"]):
        ...
        providers[provider_id] = {
            "profiles": sorted(adapter.get("profiles", {})),
            "models": [
                row["model_id"]
