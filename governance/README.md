...
32K 思考候选包、S-07-G-A 的 Z66 三问法历史候选包。六份模型包分别有
...
S-07-G-A 只建同盘完整副本和轻量入口，没有移动或删除 93 个主仓源文件。若另开
S-07-G-B-A，预计可在保留轻量说明、四张身份票和这份请求 JSON 后移除 88 个重件；必须先做
一次完整取回和消费者复验，不能由本节自动授权。
