...
28 个文件／550,603 字节、40 个文件／702,031 字节、41 个文件／714,081 字节。三份
7 月 23 日旧清单缺逐项 SHA，旧 TEMP 外置根没有清单，
...
## S-07-D-A／D-B LongCat r03 中断硬停轮瘦身

这一波只从固定提交 `62d942e6cf05c7722826c1648436a2eb15708563` 复制 r03 的 28 个
普通文件到同级外置仓，原仓目录 tree 是 `0b005bfed980cf0e30210d1690bdcb2d0be67bb7`。
机器对象编号是 `model-benchmark-longcat-r03-interrupted-s07da-v1`。

历史票据说明：进程在写入首次尝试占用票后异常终止；本地模型回答数是 0，但网络结果未知，
没有 usage，也不能登记模型质量分。复制动作本身没有联网或调用模型。

S-07-D-A 只建立副本。S-07-D-B 又完成整包取回逐字节比对，并确认 `tests/`、`tools/`、
`config/` 对 r03 精确编号的消费者是 0；随后从主仓移除 24 个已封存重件、552,370 字节。
主仓仍保留身份、状态、两份硬停证据和结论卡，因此总索引与 r02 历史替代指针没有断。

## S-07-E-A／E-B-A／E-B-B LongCat r01 中断硬停轮瘦身

这一波从固定提交 `0550e45c9c4cafbda905e8d436f883d04c5e818b` 复制 r01 的 28 个
普通文件到同级外置仓，原仓目录 tree 是 `130d9cda5b73041ab83453ca639d3eb8c1ae2a1b`。
机器对象编号是 `model-benchmark-longcat-r01-interrupted-s07ea-v1`。

r01 在首次尝试占用票后被 SIGKILL 终止；本地回答数是 0，但网络结果和用量未知，没有
质量分，也禁止原地重跑。复制动作没有调用模型或联网。

S-07-E-A 不移动或删除主仓文件。S-07-E-B-A 把旧版 `neutral_extract.py` 的原字节
复制到 `tests/fixtures/z57_frozen_neutral_extract_20260723/`，并让
`tests/test_z57_default_contract_hotfix.py` 从专用夹具取件。夹具仍核对原 SHA，
不冒充现役程序；S-07-E-B-A 当时也没有删除 r01 的 28 个文件。精确消费者和测试复验
通过后，重件移除另开 S-07-E-B-B 审批。S-07-E-B-B 又在外置验证和完整取回通过后，从主仓移除 24 个
重件、547,439 字节，只留身份、状态、两份硬停证据和结论卡。轻量目录不能原地运行。

## S-07-F-A 千问 r05／r06 有分候选完整副本
...
两轮消费者都保持 `open`，主仓原目录完整保留。S-07-F-A 没有迁移消费者，也没有移动或
删除主仓文件。后续若要瘦身，必须另开一波处理消费者和完整取回。
