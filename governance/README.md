...
当前白名单有 S-05-B 回放包 v1／v2、S-07-B-A 的 5 轮零调用作废模型横评完整包，以及
S-07-C-A 的 QEC 四模型终局证据包。前一份模型包来自固定 Git 提交，86 个文件、
1,664,821 字节已经逐文件回读；后一份只有 7 个原件、11,424 字节。三份 7 月 23 日旧
清单缺逐项 SHA，旧 TEMP 外置根没有清单，政策会明确排除，工具不会顺着旧绝对路径猜位置。
