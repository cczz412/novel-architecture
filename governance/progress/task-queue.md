...
- 公告（2026-08-15）：产品试跑示例已迁入本仓 [`novel-mvp/`](../../novel-mvp/README.md)。这是测试阶段示例，不是现行产品；代码偏旧，设计稿可参考。作者书稿在 `novel-mvp/data/`，不进 Git。
- 公告（2026-08-13）：产品 MVP 主办仓当时建在同级 `novel-mvp/`（CZ 拍板、Cursor 直营、轻档治理、已接真 API）。Codex 窗口**不接管、不巡检、不顺手优化**该示例，除非 CZ 点名派单；研究结论照旧只在本仓沉淀。
