#!/usr/bin/env python3
"""Score only the token-weighted-reducer-diagnostic READ1 L6 qualification probe."""
...
OLD_DENSITY_FAIL_TICKET = (
    REPO
    / "runs/T5_R04_READ1_TRAIN72_DENSITY_PAIRED_R01/L6_MECHANICAL_FAIL_TICKET.json"
)
...
VARIANTS = ("BASE_L6", "TOKEN_WEIGHTED_DIAGNOSTIC_LORA_L6")
...
def load_core() -> Any:
    ...
    module_spec = importlib.util.spec_from_file_location("token_weighted_diagnostic_scoring_core", CORE_SCORER)
...
def runtime_rows() -> tuple[list[dict[str, Any]], str]:
    ...
    lora_path = RUN_ROOT / "inference/L6_TOKEN_WEIGHTED_DIAGNOSTIC_LORA_RAW_6.jsonl"
    ...
    if len(lora) != 6 or [row.get("case_id") for row in lora] != CASE_ORDER:
        raise RuntimeError("TOKEN_WEIGHTED_DIAGNOSTIC_LORA_L6_DENOMINATOR_OR_ORDER_DRIFT")
    if any(row.get("variant") != "TOKEN_WEIGHTED_DIAGNOSTIC_LORA_L6" for row in lora):
        raise RuntimeError("TOKEN_WEIGHTED_DIAGNOSTIC_LORA_L6_VARIANT_DRIFT")
...
def mechanical_report(parsed: list[dict[str, Any]], combined_sha: str) -> dict[str, Any]:
    lora = [row for row in parsed if row["variant"] == "TOKEN_WEIGHTED_DIAGNOSTIC_LORA_L6"]
    ...
    return {
        ...
        "token_weighted_diagnostic_lora_raw_sha256": sha256(
            RUN_ROOT / "inference/L6_TOKEN_WEIGHTED_DIAGNOSTIC_LORA_RAW_6.jsonl"
        ),
...
def run_pre() -> None:
    ...
    if not mechanical["pass"]:
        ...
        gate_path = output / "L6_MECHANICAL_GATE.json"
        write_json(
            RUN_ROOT / "L6_MECHANICAL_FAIL_TICKET.json",
            {
                "schema_version": "read1-train72-token-weighted-reducer-l6-mechanical-fail/1.0",
                "status": "FAIL_L6_MECHANICAL_GATE",
                "stopped_at": datetime.now(ZoneInfo("Asia/Shanghai")).isoformat(timespec="seconds"),
                "run_id": "T5_R04_READ1_TRAIN72_TOKEN_WEIGHTED_REDUCER_DIAGNOSTIC_R01",
                "failure_reason": [name for name, passed in mechanical["checks"].items() if not passed],
                "bindings": {
                    "spec_sha256": sha256(SPEC_PATH),
                    "runner_sha256": sha256(RUNNER),
                    "scorer_sha256": sha256(Path(__file__)),
                    "authorization_ticket_sha256": sha256(RUN_ROOT / "AUTHORIZATION_TICKET.json"),
                    "run_identity_sha256": sha256(RUN_ROOT / "RUN_IDENTITY.json"),
                    "training_result_sha256": sha256(RUN_ROOT / "TRAINING_RESULT.json"),
                    "l6_raw_sha256": mechanical["token_weighted_diagnostic_lora_raw_sha256"],
                    "mechanical_gate_sha256": sha256(gate_path),
                    "pre_metrics_sha256": sha256(metrics_path),
...
def run_final(adjudications: Path) -> None:
    ...
    lora = result["variants"]["TOKEN_WEIGHTED_DIAGNOSTIC_LORA_L6"]["semantic_recoverable"]
    gate = {
        "dataset": "L6",
        "pass": lora["f1"] >= base["f1"],
        "check": "token_weighted_diagnostic_lora_f1_not_below_same_contract_reused_base",
        "base_semantic": base,
        "token_weighted_diagnostic_lora_semantic": lora,
        "spec_sha256": sha256(SPEC_PATH),
        "runner_sha256": sha256(RUNNER),
        "scorer_sha256": sha256(Path(__file__)),
        "real24_run": False,
