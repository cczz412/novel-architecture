#!/usr/bin/env python3
"""Run one ticket-bound READ1 TRAIN72 zero-low-paired qualification."""
...
SPEC_PATH = EXP / "ZERO_LOW_PAIRED_SPEC.json"
TRAIN = EXP / "READ_1_TARGET_TRAIN72_ZERO_LOW_PAIRED.jsonl"
PAIR_MAP = EXP / "ZERO_LOW_PAIR_MAP.json"
SCORER = EXP / "score_zero_low_paired_l6.py"
RUN_ROOT = REPO / "runs/T5_R04_READ1_TRAIN72_ZERO_LOW_PAIRED_R01"
SOURCE_TRAIN = (
    REPO
...
DENSITY_RUN = REPO / "runs/T5_R04_READ1_TRAIN72_DENSITY_PAIRED_R01"
DENSITY_FAIL_TICKET = DENSITY_RUN / "L6_MECHANICAL_FAIL_TICKET.json"
...
TRAIN_SHA = "d0448165b740a35f2a0460a32e3c07eea4d8459dbb66df7d473b1a6ea5613ed6"
PAIR_MAP_SHA = "73d6d327cd14aff1fdedaeb87a2a79f419606bc6c28efec04092c4e15f3ab986"
...
DENSITY_FAIL_TICKET_SHA = "b0674dcf87a355798659f893f650f397c6c329c60d34898c58778ce5f6ca7146"
...
TICKET_KEYS = {
    ...
    "density_paired_fail_ticket_sha256",
...
def static_check() -> dict[str, Any]:
    ...
    fixed = {
        ...
        DENSITY_FAIL_TICKET: DENSITY_FAIL_TICKET_SHA,
    ...
    zero_checks = pair_map.get("zero_pair_checks", {})
    if zero_checks != {
        "zero_with_one_or_two_batches": 7,
        "zero_with_zero_batches": 0,
        "zero_with_three_or_more_batches": 0,
    }:
        raise RuntimeError("PAIR_MAP_ZERO_LOW_CONTRACT_DRIFT")
...
def validate_ticket(ticket_path: Path, command: str) -> tuple[dict[str, Any], bytes]:
    ...
    if ticket["schema_version"] != "read1-train72-zero-low-paired-execution-ticket/1.0":
        raise RuntimeError("CZ_CONTROL_WINDOW_TICKET_SCHEMA_INVALID")
    if ticket["scope"] != "READ1_TRAIN72_ZERO_LOW_PAIRED_SINGLE_RUN":
        raise RuntimeError("CZ_CONTROL_WINDOW_TICKET_SCOPE_INVALID")
    if ticket["run_id"] != "T5_R04_READ1_TRAIN72_ZERO_LOW_PAIRED_R01":
    ...
    expected = {
        ...
        "density_paired_fail_ticket_sha256": DENSITY_FAIL_TICKET_SHA,
...
def load_runtime() -> Any:
    module_spec = importlib.util.spec_from_file_location("read1_zero_low_paired_runtime", RUNTIME_RUNNER)
...
def prepare_run_files(ticket: dict[str, Any], ticket_raw: bytes) -> None:
    ...
    (RUN_ROOT / "config/zero_low_paired.yaml").write_text(
    ...
    write_json(
        RUN_ROOT / "RUN_IDENTITY.json",
        {
            ...
            "density_paired_fail_ticket_sha256": sha256(DENSITY_FAIL_TICKET),
...
def train(ticket_path: Path) -> None:
    ...
    command = [str(PYTHON), "-m", "mlx_lm.lora", "--config", str(RUN_ROOT / "config/zero_low_paired.yaml")]
    ...
    with (RUN_ROOT / "training.log").open("x", encoding="utf-8") as log:
        ...
        for line in child.stdout:
            ...
            if match:
                ...
                print(f"ZERO_LOW_PAIRED_PROGRESS iteration={match.group(1)}/36 memory_free={free}%", flush=True)
                if free < 8:
                    child.terminate()
                    child.wait()
    ...
    write_json(
        ...
        {
            "status": "ZERO_LOW_PAIRED_TRAINED_FINAL_ONLY",
            "iterations": 36,
            "optimizer_updates": 9,
...
def verify_trained_state(ticket_raw: bytes) -> None:
    ...
    if result.get("status") != "ZERO_LOW_PAIRED_TRAINED_FINAL_ONLY":
...
def infer_l6(ticket_path: Path) -> None:
    ...
    output = RUN_ROOT / "inference/L6_ZERO_LOW_PAIRED_LORA_RAW_6.jsonl"
    if output.exists():
    ...
    for index, row in enumerate(rows, 1):
        ...
        result = runtime.generate_one(
            ...
            stream_generate,
            sampler,
            "ZERO_LOW_PAIRED_LORA_L6",
            case_id,
            row["messages"],
            output,
        )
        print(
            f"ZERO_LOW_PAIRED_L6_INFER case={case_id} observed={index}/6 "
...
def status() -> None:
    ...
    for name in (
        ...
        "inference/L6_ZERO_LOW_PAIRED_LORA_RAW_6.jsonl",
