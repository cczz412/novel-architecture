#!/usr/bin/env python3
"""Score only the zero-low-paired READ1 L6 qualification probe."""
...
RUNNER = EXP / "run_zero_low_paired.py"
...
VARIANTS = ("BASE_L6", "ZERO_LOW_PAIRED_LORA_L6")
...
def load_core() -> Any:
    ...
    module_spec = importlib.util.spec_from_file_location("zero_low_paired_scoring_core", CORE_SCORER)
...
def runtime_rows() -> tuple[list[dict[str, Any]], str]:
    ...
    lora_path = RUN_ROOT / "inference/L6_ZERO_LOW_PAIRED_LORA_RAW_6.jsonl"
    ...
    if len(lora) != 6 or [row.get("case_id") for row in lora] != CASE_ORDER:
        raise RuntimeError("ZERO_LOW_PAIRED_LORA_L6_DENOMINATOR_OR_ORDER_DRIFT")
    if any(row.get("variant") != "ZERO_LOW_PAIRED_LORA_L6" for row in lora):
        raise RuntimeError("ZERO_LOW_PAIRED_LORA_L6_VARIANT_DRIFT")
...
def mechanical_report(parsed: list[dict[str, Any]], combined_sha: str) -> dict[str, Any]:
    lora = [row for row in parsed if row["variant"] == "ZERO_LOW_PAIRED_LORA_L6"]
    ...
    return {
        ...
        "zero_low_paired_lora_raw_sha256": sha256(
            RUN_ROOT / "inference/L6_ZERO_LOW_PAIRED_LORA_RAW_6.jsonl"
        ),
...
def run_final(adjudications: Path) -> None:
    ...
    lora = result["variants"]["ZERO_LOW_PAIRED_LORA_L6"]["semantic_recoverable"]
    gate = {
        "dataset": "L6",
        "pass": lora["f1"] >= base["f1"],
        "check": "zero_low_paired_lora_f1_not_below_same_contract_reused_base",
        "base_semantic": base,
        "zero_low_paired_lora_semantic": lora,
