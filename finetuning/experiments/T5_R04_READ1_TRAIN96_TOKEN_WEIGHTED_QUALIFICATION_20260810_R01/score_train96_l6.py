#!/usr/bin/env python3
"""Score only the ticketed TRAIN96 READ1 L6 qualification probe."""
...
RUNNER = EXP / "run_train96_token_weighted.py"
...
LORA_VARIANT = "TRAIN96_TOKEN_WEIGHTED_LORA_L6"
...
def runtime_rows() -> tuple[list[dict[str, Any]], str]:
    ...
    lora_path = RUN_ROOT / "inference/L6_TRAIN96_TOKEN_WEIGHTED_LORA_RAW_6.jsonl"
...
def run_pre() -> None:
    ...
    if not mechanical["pass"]:
        ...
        parent.write_json(
            ...
            {
                "schema_version": "read1-train96-token-weighted-l6-mechanical-fail/1.0",
                "status": "FAIL_L6_MECHANICAL_GATE",
        ...
        return
    result = parent.semantic_score(parsed, combined_sha, gold, units, None)
...
def run_final(adjudications: Path) -> None:
    ...
    if output.exists():
        raise RuntimeError(f"SCORING_OUTPUT_EXISTS_NO_OVERWRITE:{output}")
    parsed, combined_sha, gold, units = parent.parsed_case_rows()
    result = parent.semantic_score(parsed, combined_sha, gold, units, adjudications)
    queue = result.pop("blind_queue")
