#!/usr/bin/env python3
"""Score the Stage 2 L6 probe, with a mechanical gate before blind review."""
...
VARIANTS = ("BASE_L6", "STAGE2_LORA_L6")
...
def load_core() -> Any:
    ...
    module_spec = importlib.util.spec_from_file_location("read1_long_sm12_stage2_core", CORE_SCORER)
...
def runtime_rows() -> tuple[list[dict[str, Any]], str]:
    ...
    lora_path = RUN_ROOT / "inference/L6_STAGE2_LORA_RAW_6.jsonl"
    ...
    if len(lora) != 6 or [row.get("case_id") for row in lora] != CASE_ORDER:
        raise RuntimeError("STAGE2_LORA_L6_DENOMINATOR_OR_ORDER_DRIFT")
    if any(row.get("variant") != "STAGE2_LORA_L6" for row in lora):
        raise RuntimeError("STAGE2_LORA_L6_VARIANT_DRIFT")
    combined = compact_jsonl(base + lora)
    return base + lora, hashlib.sha256(combined).hexdigest()


def parsed_case_rows() -> tuple[list[dict[str, Any]], str, dict[str, list[dict[str, Any]]], dict[str, list[dict[str, Any]]]]:
    static_check()
    core = load_core()
...
def mechanical_report(parsed: list[dict[str, Any]], combined_sha: str) -> dict[str, Any]:
    lora = [row for row in parsed if row["variant"] == "STAGE2_LORA_L6"]
    prediction_counts = {row["case_id"]: row["predictions"] for row in lora}
    checks = {
        "json_6_of_6": sum(row["json_valid"] for row in lora) == 6,
        "schema_6_of_6": sum(row["schema_valid"] for row in lora) == 6,
        "zero_cases_both_empty": prediction_counts.get("LC-L01") == 0 and prediction_counts.get("LC-L02") == 0,
        "four_nonempty_cases_each_nonempty": all(prediction_counts.get(f"LC-L{i:02d}", 0) >= 1 for i in range(3, 7)),
    ...
    return {
        ...
        "stage2_lora_raw_sha256": sha256(RUN_ROOT / "inference/L6_STAGE2_LORA_RAW_6.jsonl"),
...
def semantic_score(
    ...
    core = load_core()
    wo = core.load_wo()
    decisions = decision_index(adjudications, combined_sha, gold) if adjudications is not None else {}
    ...
    for row in parsed:
        ...
        for prediction_index in unmatched:
            ...
            if decision is None:
                ...
                gold_index = next(i for i, fact in enumerate(expected) if fact["fact_id"] == decision["matched_gold_fact_id"])
    ...
    for variant in VARIANTS:
        ...
        variants[variant] = {
            ...
            "status_correct_rate_on_semantic_tp": round(sum(row["status_correct"] for row in group) / tp if tp else 0.0, 6),
...
def run_pre() -> None:
    ...
    if not mechanical["pass"]:
        ...
        print(json.dumps({"status": "FAIL_L6_MECHANICAL_GATE", "blind_queue_created": False}, ensure_ascii=False))
        return
...
def run_final(adjudications: Path) -> None:
    pre_gate_path = RUN_ROOT / "scoring/l6_pre/L6_MECHANICAL_GATE.json"
    if not pre_gate_path.is_file() or read_json(pre_gate_path).get("pass") is not True:
    ...
    lora = result["variants"]["STAGE2_LORA_L6"]["semantic_recoverable"]
    gate = {
        "dataset": "L6",
        "pass": lora["f1"] >= base["f1"],
        "check": "stage2_lora_f1_not_below_same_contract_reused_base",
        "base_semantic": base,
        "stage2_lora_semantic": lora,
    ...
    print(json.dumps({"status": result["status"], "semantic_pending": 0, "gate": gate["pass"]}, ensure_ascii=False))
