...
def _action(fact: dict, operation_id: str, decision: str, text: str | None = None) -> dict:
...
def test_confirm_reject_rejudge_and_atomic_edit_confirm(tmp_path: Path) -> None:
    ...
    assert reject["before_status"] == "confirmed" and reject["after_status"] == "rejected"

    rejected = _read_json(root / "facts.json")[0]
    edited = factstore.review_fact(
        root,
        action=_action(rejected, "op-edit-confirm-01", "edit_and_confirm", "林乔把钥匙交给苏晚。"),
    ...
    assert [row["action"] for row in _read_jsonl(root / "commit_log.jsonl") if row["phase"] == "prepare"] == [
        "fact_review",
        "fact_review",
        "fact_review",
    ]
...
def test_legacy_store_and_cli_surface_delegate_to_transaction_writer(
    ...
    assert store.add_fact_candidates(
        "book", chapter["id"], [{"text": "林乔拿起钥匙。", "quote": "林乔拿起钥匙。"}], "fixture"
    ) == 1
...
def test_legacy_duplicate_id_repair_uses_same_transaction_coordinator(
    ...
    prepare = next(row for row in _read_jsonl(root / "commit_log.jsonl") if row["phase"] == "prepare")
    assert prepare["action"] == "fact_id_repair"
    assert {item["path"] for item in prepare["files"]} == {"facts.json", "repair_ids_report.json"}
...
def test_rejudge_invalidates_old_actual_support_and_does_not_revive_it(tmp_path: Path) -> None:
    root = tmp_path / "book"
    ...
    assert _read_json(root / "plan.json")["reconciliation_edges"][0]["edge_status"] == "stale"
...
def test_cross_file_fault_recovery(tmp_path: Path, fault_at: str, expected_state: str) -> None:
    ...
    assert planstore.operation_status(root, action["operation_id"])["state"] == expected_state
...
def test_cross_process_competing_rejudgments_cannot_both_commit(tmp_path: Path) -> None:
    ...
    original = _init(root)
    action_paths = []
    for operation_id, decision in (("op-race-confirm", "confirm"), ("op-race-reject", "reject")):
...
def test_direct_facts_writer_is_confined_to_factstore_and_bootstrap() -> None:
    ...
    assert store_text.count('_save(d / "facts.json"') == 1  # 只剩 init_project 的空账初始化
    assert 'f["status"] =' not in store_text


def test_contract_and_docs_name_the_unique_writer() -> None:
    contract = (MVP_ROOT / "contracts" / "FACT_REVIEW_ACTION.md").read_text(encoding="utf-8")
