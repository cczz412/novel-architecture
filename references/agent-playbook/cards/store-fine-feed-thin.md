...
账本里可以记细。每次生成只编译本次所需最小上下文，每项可回答「为什么加载」。很多概念是**存而不喂**（账本里记着，生成时不装）。「瘦」＝最小充分＋可扩张，不是越小越好。续写必须带上共同前提（历史事实、规划限制、禁做清单、人物状态），但不能把几十上百章整本加载。不要把「整卷／整书宽窗口」写成当前已经具备的能力。

## 加固形状（SI-008，还不是合同）

调查建议：程序按规则编译瘦包；RAG 只找候选，不是真值。灵感没安放前不进执行包。缺料最多两轮受控回取，回取后整包重编，不要往旧包末尾堆。
