...
**E-13｜R2 机械召回路线的历史边界** `候选先验＋待复验`

历史施工记录显示，在 2,500 字预算的同卷零调用对照里，A4／A1c 曾是机械召回前两名；A4h 没有增加覆盖，A5 只增加了局部事实头、没有增加完整题，A6 没有收益，A7 虽增加 B01 的零散事实头，却让总体和 B02／B03 回退。

边界：当前 `main` 已没有这些运行目录，无法在本轮逐格复算；这些记录只说明几条旧候选为什么停，不证明回答质量，也不授权恢复 API 或把任一路线升为默认。若将来重开，必须从新票、冻结输入和可回放证据重新开始。

证据：迁移前文件 `governance/progress/r2-chatgpt-review-cycle.md`，保存于 `main@5b4321c8ccf68f419fc0d5de5d8bba50702a79bd`，Git blob `49669707e457706bb9418f8bdbd1c584d0458f18`；外置评审包身份见 `governance/external_archive_registry.json` 的 `w05-r2-chatgpt-development-route-review-pack-20260727-v1`；迁移回执见 GitHub #135 与 Linear CCZ-107。

**E-14｜T03 多岗位抽取管线的历史候选证据** `候选先验＋待复验`

历史施工记录留下四类可复用判断：D0 的五类原创合成材料暴露过材料身份、导入损失和入账证据字段问题；D1 的主抽小样里豆包暂留默认、V4 Flash 留作速度／资源挑战者；D2 的补漏小样里 GLM 暂留候选，DeepSeek V3 出现过格式失败；D3 的隔离副本曾把主抽、补漏、验真和去噪模型配置拆开，并用零 API 冒烟确认岗位不串位。

边界：这批记录合计登记 59 次 API 请求、0 重试，但当前 `main` 没有对应 TEMP 证据和 Gold，不能据此比较真实质量、冻结岗位模型、恢复旧并行窗口或改正式小说辅助产品默认。它只能帮助新票避免重复踩输入身份、证据承载和岗位串位的问题。

证据：迁移前文件 `governance/progress/branches/t03-api-pipeline-debug/STATUS.md`，保存于 `main@5b4321c8ccf68f419fc0d5de5d8bba50702a79bd`，Git blob `de8cfdf099aee688a3277d965881aabea6dc9e56`；迁移回执见 GitHub #135 与 Linear CCZ-107。

---

补账规则：只有正式票直接支持的内容才能标“已验证”或“负结果”；外部回包、已缺运行实物的旧施工页只标候选先验或待复验。任何人话改写都不得扩大票的适用范围或执行权限。
